...
This someone's attempt to write the autoencoder. Mostly to get some
more experience working in Tensorflow.

sourced from:
https://github.com/GjjvdBurg/TensorFlowExperiments/blob/master/autoencoder/mnist_autoencoder.py
    ...
    with tf.Session() as sess:
        ...
        if train :
            ...
            for i in range(int(20001)):
                ...
                feed = {x:batch[0]}
                if i % 500 == 0:
                    test_loss = sess.run(loss,feed_dict=feed)
                    test_latent = sess.run(latent,feed_dict=feed)
                    print("step %d, training loss: %g" % (i, test_loss))
                    print "step",i,"training loss: ",test_latent[0]
